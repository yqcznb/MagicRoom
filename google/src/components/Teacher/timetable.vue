      ...
      <div class="timetable">
          <div class="table">
            <div :style="{ height: '360px' }">
              <van-slider v-model="value" disabled @change="onSlider" vertical />
            </div>
            <div class="time">
              <ul> 
            ...
            <div class="name">
              <ul> 
                <li v-for="(item,index) in names" :key="index">
                  <div class="classname" v-if="!item.time">{{item.name}}<span>{{item.xq}}</span></div>
                  <div class="classname time-color" v-else>{{item.name}}</div>
                </li>
              </ul>
            </div>
          </div>
          <div class="label">
            <ul> 
              <li v-for="(item,index) in label" :key="index" :class="{'active':week==index}" @click="weekBtn(index)">
...
export default {
  data () {
    return {
      ...
      names: [
        {name: '清晨',xq: '',time:true},
        {name: '前端框架应用',xq: '详情》'},
        {name: '前端框架应用',xq: '详情》'},
        {name: '前端框架应用',xq: '详情》'},
        {name: '前端框架应用',xq: '详情》'},
        {name: '中午',xq: '',time:true},
        {name: '前端框架应用',xq: '详情》'},
        {name: '前端框架应用',xq: '详情》'},
        {name: '前端框架应用',xq: '详情》'},
        {name: '前端框架应用',xq: '详情》'},
        {name: '傍晚',xq: '',time:true},
        {name: '前端框架应用',xq: '详情》'},
        {name: '前端框架应用',xq: '详情》'},
      ],
      label: [
  ...
  created(){
    ...
    if(myDate.getHours()<8){
    ...
    }else if(myDate.getHours()==9){
      if(myDate.getMinutes()<20){
        this.value = 6
      }else{
        this.value = 14
      }
      ...
      if(myDate.getMinutes()<15){
        this.value = 22
      }else{
        this.value = 28
      }
    }else if(myDate.getHours()==12 || myDate.getHours()==13){
        this.value = 37
      ...
      .timetable{
          ...
          .table{
            ...
            .name{
              width: 65%;
              .time-color{
                background-color: #cfebc2!important;
                padding: 0px 4em;
                margin-top: 5px;
              }
              ul{
